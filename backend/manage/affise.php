<?php 
    ...
    $db = new DB();
    $prefix = $db->getPrefix();
    $totalRecords = (int)$db->query("SELECT COUNT(*) as total FROM `".$prefix."affise`")[0]['total'];
    $pages = ceil($totalRecords / $perpage);
    $where = '';
    if(!empty($search)){
        $where = "WHERE `sub1` LIKE '%$search%' OR `sub2` LIKE '%$search%' OR `sub1Fake` LIKE '%$search%' OR `offer_id` LIKE '%$search%' OR `title` LIKE '%$search%'";
    }
    $data = $db->query("SELECT * FROM `".$prefix."affise` $where ORDER BY `id` DESC LIMIT ".$offset.",".$perpage."");
?>

<h2 class="text-center">Manage Affise Offers</h2>
